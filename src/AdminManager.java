...
public class AdminManager {
    ...
    public void showAdminMenu() {
        while (true) {
            ...
            // System.out.println("13. View Hive Statistics (Parallel)");
            // System.out.println("14. View Task Statistics (Parallel)");
        ...
        switch (choice) {
            ...
            case "4":
                ...
                try {
                    ...
                    logger.logActivity(adminUser.getId(), adminUser.getName(), 
                                      "Updated task due date - Task ID: " + taskId);
                } catch (Exception e) {
    ...
    private void viewReports() {
        ...
        Map<Integer, Hive> allHives = dataManager.getHives();
        Map<Integer, Task> allTasks = dataManager.getTasks();
        Map<Integer, User> allUsers = dataManager.getUsers();
        Map<Integer, Report> allReports = dataManager.getReports();
        
        if (!allReports.isEmpty()) {
            List<Report> reportsList = new ArrayList<>(allReports.values());
            // Sort reports by timestamp (newest first)
            reportsList.sort((r1, r2) -> r2.getTimestamp().compareTo(r1.getTimestamp()));
            
            for (Report report : reportsList) {
                System.out.println("\nReport #" + report.getId() + " by " + report.getUserName() + 
                                 " at " + formatTimestamp(report.getTimestamp()));
                System.out.println("Content: " + report.getContent());
                
                // Display related hives
                if (!report.getRelatedHiveIds().isEmpty()) {
                    System.out.print("Related Hives: ");
                    for (int i = 0; i < report.getRelatedHiveIds().size(); i++) {
                        int hiveId = report.getRelatedHiveIds().get(i);
                        Hive hive = allHives.get(hiveId);
                        if (hive != null) {
                            System.out.print("Hive #" + hiveId + 
                                           " (Healthy: " + (hive.isHealthy() ? "Yes" : "No") + 
                                           ", Needs Attention: " + (hive.isNeedsAttention() ? "Yes" : "No") + 
                                           ", Queenless: " + (hive.isQueenless() ? "Yes" : "No") + 
                                           ", Honey: " + String.format("%.1f", hive.getHoneyLevel()) + "%)");
                        } else {
                            System.out.print("Hive #" + hiveId + " (Not found)");
                        }
                        if (i < report.getRelatedHiveIds().size() - 1) {
                            System.out.print(", ");
                        }
                    }
                    System.out.println();
                ...
                if (!report.getRelatedTaskIds().isEmpty()) {
                    System.out.print("Related Tasks: ");
                    for (int i = 0; i < report.getRelatedTaskIds().size(); i++) {
                        int taskId = report.getRelatedTaskIds().get(i);
                        ...
                        if (task != null) {
                            ...
                            System.out.print("Task #" + taskId + " (Not found)");
                        }
                        if (i < report.getRelatedTaskIds().size() - 1) {
                            System.out.print(", ");
                        }
                    }
                    System.out.println();
        ...
        logger.logActivity(adminUser.getId(), adminUser.getName(), "Viewed user reports");
    }
    ...
    private void deleteReport() {
        ...
        System.out.println("Created by: " + report.getUserName());
        System.out.println("Created at: " + report.getTimestamp());
        System.out.println("Content: " + report.getContent());
        ...
        if (!confirmation.equals("y") && !confirmation.equals("yes")) {
            System.out.println("Report deletion cancelled.");
            return;
        }
    ...
    private String formatTimestamp(String timestamp) {
        try {
            // Parse the ISO timestamp and format it to show only date and time up to seconds
    ...
    /*
    private void viewHiveStatisticsParallel() {
        System.out.println("\n=== Hive Statistics (Parallel Processing) ===");
        
        // Get all hives
        Map<Integer, Hive> hivesMap = dataManager.getHives();
        if (hivesMap.isEmpty()) {
            System.out.println("No hives found.");
            logger.logActivity(adminUser.getId(), adminUser.getName(), "Viewed hive statistics - No hives found");
            return;
        }
        
        List<Hive> hivesList = new ArrayList<>(hivesMap.values());
        
        // Use parallel processing to generate statistics
        int numThreads = Runtime.getRuntime().availableProcessors();
        
        // Display the statistics
        
        logger.logActivity(adminUser.getId(), adminUser.getName(), 
                          "Viewed hive statistics - Parallel processing not available");
    }
    
    private void viewTaskStatisticsParallel() {
        System.out.println("\n=== Task Statistics (Parallel Processing) ===");
        
        // Get all tasks
        Map<Integer, Task> tasksMap = dataManager.getTasks();
        if (tasksMap.isEmpty()) {
            System.out.println("No tasks found.");
            logger.logActivity(adminUser.getId(), adminUser.getName(), "Viewed task statistics - No tasks found");
            return;
        }
        ...
        logger.logActivity(adminUser.getId(), adminUser.getName(), 
                          "Viewed task statistics - Parallel processing not available");
    }
    */
